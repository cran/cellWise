...
vignette: >
  %\VignetteIndexEntry{DI examples}
...
In this example we consider an artificial dataset with cellwise outliers.
First we construct a correlation matrix and then use it to generate the data.
```{r}
d     <- 10
mu    <- rep(0, 10)
...
Now extract the VOC data and run the DI algorithm to estimate a covariance matrix
and flag cellwise outliers.
```{r}
